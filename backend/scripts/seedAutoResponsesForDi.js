...
const mockSenders = [
  { name: "Jean Dupont", email: "jean.dupont@example.com" }, { name: "Marie Curie", email: "marie.curie@science.org" },
  { name: "Paul Martin", email: "p.martin@entreprise-fictive.fr" }, { name: "Support Client", email: "client.support@grandgroupe.com" },
  { name: "Alice Wonderland", email: "alice@wonderland.net" }, { name: "Bob Builder", email: "bob@builder.co.uk" }
];

const mockContents = [
  "Bonjour, je souhaiterais obtenir un devis pour le projet mentionné en objet. Merci.", "Est-ce que 14h vous convient pour notre point de demain ?",
  "Je ne comprends pas la ligne 3 de la facture reçue ce matin.", "Avez-vous 5 minutes pour discuter de la stratégie ?",
  "Super boulot de l'équipe hier, tout fonctionne à merveille.", "Impossible de me connecter ce matin, message d'erreur 'Token Invalid'.",
  "Nous organisons un webinaire sur les tendances 2026, inscrivez-vous !", "Voici notre nouveau RIB en pièce jointe pour les prochains virements.",
  "La commande n'est toujours pas arrivée, pouvez-vous vérifier ?", "Bravo pour la mise en ligne, le site est magnifique.",
  "Le serveur de prod est lent, pouvez-vous jeter un œil ?", "Notre contrat expire le mois prochain, quelles sont les options ?",
  "Nous aimerions discuter d'une collaboration potentielle.", "J'ai remarqué un petit bug sur le bouton 'Envoyer'.",
  "Désolé, je dois reporter notre call à la semaine prochaine.", "Où puis-je trouver la doc pour l'endpoint /users ?",
  "La page de login renvoie une 404 depuis ce matin.", "Est-il prévu d'ajouter une intégration avec Slack ?",
  "Sauf erreur de notre part, la facture de novembre est impayée.", "Veuillez noter que nos tarifs augmenteront de 2% au 1er janvier.",
  "Je vous envoie mon CV pour le poste de Fullstack JS.", "Comment sont traitées mes données personnelles ?",
  "Mon compte semble verrouillé après 3 tentatives.", "Ce serait bien d'avoir un mode sombre sur l'app mobile.",
...
const seedForDi = async () => {
  try {
    await connectDB();

    console.log("\n🚀 Démarrage du script de seed pour 'Di'...");

    // 1. Créer ou récupérer le Tenant "Di"
    let tenant = await Tenant.findOne({ companyName: "Di" });
    if (!tenant) {
      ...
      console.log("✅ Tenant 'Di' créé avec succès.");
    } else {
      console.log(`✅ Tenant 'Di' existant trouvé (ID: ${tenant._id})`);
    }

    // 2. Créer ou récupérer l'utilisateur "durelzanfack@gmail.com"
    let user = await User.findOne({ email: "durelzanfack@gmail.com" });
    if (!user) {
      ...
      console.log("✅ Utilisateur 'durelzanfack@gmail.com' créé et lié à 'Di'.");
    } else {
      // S'assurer qu'il est lié au bon tenant
      if (user.tenant_id.toString() !== tenant._id.toString()) {
        user.tenant_id = tenant._id;
        await user.save();
        console.log("🔄 Utilisateur relié au tenant 'Di'.");
      } else {
        console.log("✅ Utilisateur déjà lié correctement à 'Di'.");
      }
    }

    // 3. Injecter les 25 emails pour cet utilisateur
    const communications = [];
    for (let i = 0; i < 25; i++) {
      const sender = mockSenders[Math.floor(Math.random() * mockSenders.length)];
      const date = new Date();
      date.setDate(date.getDate() - Math.floor(Math.random() * 7)); // 7 derniers jours
      date.setHours(Math.floor(Math.random() * 24), Math.floor(Math.random() * 60));

      // Urgence Low ou Medium pour Auto Response
      ...
      communications.push({
        ...
        externalId: `seed-di-${Date.now()}-${i}`,
        isRead: false,
        sender: { name: sender.name, email: sender.email },
        recipient: { email: user.email },
        subject: mockSubjects[i],
        content: mockContents[i],
        snippet: mockContents[i].substring(0, 100),
        status: "To Validate",
        hasAutoResponse: false,
        hasBeenReplied: false,
        autoActivation: 'auto',
        ai_analysis: {
          summary: `Résumé : ${mockContents[i]}`,
          sentiment: Math.random() > 0.5 ? "Positive" : "Neutral",
          urgency: urgency,
          requiresResponse: true,
          suggestedResponse: mockSuggestedResponses[i], // La clé pour l'onglet Auto Response
    ...
    console.log(`✅ ${communications.length} emails injectés pour 'Di' / 'durelzanfack@gmail.com' !`);
